...
func canDeleteFolder(path string) (int, string) {
	...
	if err == nil {
		user := os.Geteuid()
		err = filepath.Walk(path, func(path string, stat fs.FileInfo, err error) error {
			if err != nil {
				return err
			} else if stat.IsDir() {
	...
	} else if runtime.GOOS != "windows" { // TODO: No Windows support for now.
		return 500, "Internal Server Error! " + err.Error()
	}
	return 0, ""
}
...
func performRemoveOperation(operation *fileOperation) (int, string) {
	...
	errStatus, errMsg := canDeleteFolder(operation.Path)
	if errStatus != 0 {
